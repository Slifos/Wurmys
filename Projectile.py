...
class Fleche(pygame.sprite.Sprite) :
    def __init__(self,lanceur,vitesse_x,vitesse_y,position_depart_x,position_depart_y,T_joueur):
        ...
        self.gravity = self.h*0.25
        self.vitesse_x = vitesse_x*1.5
        self.vitesse_y = -vitesse_y*1.5
        ...
        print(self.position_depart)
        self.damage=10
        self.t = 0

    def update(self,option):
        self.t+=0.01
        self.fleche.rect.x += self.vitesse_x*self.t
        self.fleche.rect.y -= self.vitesse_y*self.t -(1/2)*self.gravity*self.t*self.t
        for joueur in self.T_joueur:
            if joueur!=self.lanceur:
                if joueur.worm.rect.collidepoint(self.fleche.rect.x,self.fleche.rect.y):
                    ...
                    play_sound("sound/arrow_hit.mp3", 0.5, option.volume)
                    self.t=100
